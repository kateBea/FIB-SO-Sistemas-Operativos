...
int main(void)
{
    ...
    int fd_write = creat("salida.inv", S_IWUSR | S_IRUSR);
    ...
    off_t CURRENT_END = lseek(fd_read, 0, SEEK_END);
    ...
    int counter = 0;
    lseek(fd_read, CURRENT_END-1-counter, SEEK_SET);
    ret = read(fd_read, buffer, sizeof(buffer));
    bool first = true;
    while ((ret > 0) && (counter < CURRENT_END))
    {
        if (first && buffer[0] == '\n') ret = write(fd_write, buffer+1, ret);
        else                           ret = write(fd_write, buffer, ret);
        ++counter;
